...
public class Common extends Activity {
	...
	public static String fmtDate(int year, int month, int day) {
		String returnValue = "";
		returnValue = (new StringBuilder()).append(year).append("-").append(
	...
	public static String fmtDate(Calendar c) {
		...
		return fmtDate(c.get(Calendar.YEAR), c.get(Calendar.MONTH) + 1, c
	...
	public static String fmtTime(int hour, int minute) {
		...
		returnValue = (new StringBuilder())
				.append(hour > 9 ? hour : "0" + hour).append(":").append(
	...
	/*
	 * private boolean haveInternet(){ NetworkInfo
	 * info=(ConnectivityManager)getSystemService
	 * (Context.CONNECTIVITY_SERVICE).getActiveNetworkInfo(); if(info==null ||
	 * !info.isConnected()){ return false; } if(info.isRoaming()){ //here is the
	 * roaming option you can change it if you want to disable internet while
	 * roaming, just return false return true; } return true; }
	 */

	/*
	 * 
	 * String filepath ="/sdcard/play2.mp3"; File ringtoneFile = new
	 * File(filepath);
	 * 
	 ...
	 * 
	 * 
	 * //Insert it into the database Log.i(TAG,
	 * "the absolute path of the file is :"+ringtoneFile.getAbsolutePath()); Uri
	 * uri =
	 * MediaStore.Audio.Media.getContentUriForPath(ringtoneFile.getAbsolutePath
	 * ()); Uri newUri = context.getContentResolver().insert(uri, content);
	 * ringtoneUri = newUri; Log.i(TAG,"the ringtone uri is :"+ringtoneUri);
	 * RingtoneManager
