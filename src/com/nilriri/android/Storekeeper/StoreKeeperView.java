...
public class StoreKeeperView extends MapView implements
		OnCreateContextMenuListener {

	private HistoryDaoImpl dao = new HistoryDaoImpl(getContext(),
	...
	private void drawMap(int difficulty) {
		...
		if (mLevel <= endLevel) {
			...
			Toast.makeText(getContext(), res.getString(R.string.msg_ending),
		...
		if ("".equals(mapStr) || mapStr.length() < 400) {
			...
			if (mMsgText != null) {
				mMsgText.setText("Level : " + this.getLevel() + " / "
						+ endLevel);
				mMsgText.setVisibility(View.VISIBLE);
			}
		}

		Cursor c = dao.queryHistory2(Prefs.getDifficultly(storeKeeperMain),
				mLevel);
		...
		for (int x = 0; x < map.length && x < mXTileCount; x++) {
			for (int y = 0; y < map[0].length() && y < mYTileCount; y++) {
				...
				} else if (itemIndex == Common.NO || itemIndex == Common.YES
						|| itemIndex == Common.HOME || itemIndex == Common.LINE) {
	...
	private boolean isValidMove(MapItem olditem, MapItem nextitem) {
		...
		if ((olditem.state == Common.HOME || olditem.state == Common.BLANK)
				|| (nextitem.state == Common.BLANK || nextitem.state == Common.HOME)) {
			return true;
		}
	...
	public boolean moveStorekeeper(boolean isPlay) {
		...
		// â�������� ���� �Űܰ� ��ġ.
		tmpStorekeeper = new StoreKeeper(mStorekeeper.x + newx, mStorekeeper.y
				+ newy);
		...
		// ���� �������� �з��������ϸ� �з��������� ��ġ�� �ִ� ������
		MapItem nextItem = getMapItembyStorekeeper(new MapItem(
				oldItem.x + newx, oldItem.y + newy, Common.BLANK));
		...
		if (!isValidMove(oldItem, nextItem)) {
			// Log.e(TAG, "Return...");
			startAnimation(AnimationUtils.loadAnimation(storeKeeperMain,
					R.anim.shake));
		...
		// ���� �������� ���¿� ��� ���º���.
		play.add(mStorekeeper.x, mStorekeeper.y,
				getMapItembyStorekeeper(mStorekeeper).state, Common.MAN);
		...
		case Common.YES: { // â�?�ִ���
			...
			if (nextItem.state == Common.HOME) {
				...
				delMapItem(nextItem);
				mMapItemList
						.add(new MapItem(oldItem.x, oldItem.y, Common.HOME));
				mMapItemList
						.add(new MapItem(nextItem.x, nextItem.y, Common.YES));
			} else {
				play.add(oldItem.x, oldItem.y, oldItem.state, Common.HOME);
				...
				mMapItemList
						.add(new MapItem(oldItem.x, oldItem.y, Common.HOME));
				mMapItemList
						.add(new MapItem(nextItem.x, nextItem.y, Common.NO));
			}
			break;
		}
		case Common.NO: { // ���Ϳ� �ִ���
			if (nextItem.state == Common.HOME) {
				...
				delMapItem(nextItem);
				mMapItemList
						.add(new MapItem(oldItem.x, oldItem.y, Common.BLANK));
				mMapItemList
						.add(new MapItem(nextItem.x, nextItem.y, Common.YES));
			} else {
				play.add(oldItem.x, oldItem.y, oldItem.state, Common.BLANK);
				...
				mMapItemList
						.add(new MapItem(oldItem.x, oldItem.y, Common.BLANK));
				mMapItemList.add(new MapItem(nextItem.x, nextItem.y,
						oldItem.state));
			}
			break;
		}
	...
	public boolean onKeyDown(int keyCode, KeyEvent msg) {
		...
		// int difficulty =
		// storeKeeperMain.getPreferences(Context.MODE_PRIVATE).getInt(Common.PREF_KEY3,
		// Common.MEDIUM);
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		// Log.e(TAG, "onSizeChanged:w=" + w + ",h=" + h + ",oldw=" + oldw +
		// ",oldh=" + oldh);
		// int difficulty =
		// storeKeeperMain.getPreferences(Context.MODE_PRIVATE).getInt(Common.PREF_KEY3,
		// Common.MEDIUM);
	...
	public void restoreState(Bundle icicle) {
		...
		mStorekeeper = storekeeperArrayToStorekeeper(icicle
				.getIntArray("mStoreKeeper"));
	}
	...
	public void playHistory(int difficulty, int level) {
		...
		if (mHistoryCursor == null || mHistoryCursor.getCount() == 0) {
			Toast.makeText(getContext(),
					getResources().getString(R.string.msg_notfoundsavedata),
					Toast.LENGTH_LONG).show();
		} else {
	...
	private ArrayList<MapItem> stockArrayToArrayList(int[] rawArray) {
		...
		for (int index = 0; index < coordCount; index += 3) {
			MapItem c = new MapItem(rawArray[index], rawArray[index + 1],
					rawArray[index + 2]);
	...
	public void update() {
		if (mMode == RUNNING) {
			...
			if (isClearLevel()) {
				...
				case 0:
					// if (clearLevel >
					// storeKeeperMain.getPreferences(StoreKeeperMain.MODE_PRIVATE).getInt(Common.PREF_KEY0,
					// 1))
					// storeKeeperMain.getPreferences(StoreKeeperMain.MODE_PRIVATE).edit().putInt(Common.PREF_KEY0,
					// clearLevel).commit();
	...
	private Runnable mHistoryPlayer = new Runnable() {
		public void run() {
			if (mHistoryCursor.moveToNext()) {
				...
				// Log.e("mForceCheckBoxRunnable", "before=" +
				// mStorekeeper.toString());
				if (mHistoryCursor.getInt(0) > mStorekeeper.x) {
					mNextDirection = Common.RIGHT;
					// Log.e("XXXX", "mNextDirection=RIGHT" +
					// mStorekeeper.toString());
				} else if (mHistoryCursor.getInt(0) < mStorekeeper.x) {
					mNextDirection = Common.LEFT;
					// Log.e("XXXX", "mNextDirection=LEFT" +
					// mStorekeeper.toString());
				} else if (mHistoryCursor.getInt(1) > mStorekeeper.y) {
					mNextDirection = Common.DOWN;
					// Log.e("XXXX", "mNextDirection=DOWN" +
					// mStorekeeper.toString());
				} else if (mHistoryCursor.getInt(1) < mStorekeeper.y) {
					mNextDirection = Common.UP;
					// Log.e("XXXX", "mNextDirection=UP" +
					// mStorekeeper.toString());
				} else {
					// Log.e("XXXX", "=============return=================");
