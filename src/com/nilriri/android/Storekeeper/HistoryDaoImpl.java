...
public class HistoryDaoImpl extends DaoImpl {
	...
	public HistoryDaoImpl(Context context, String name, CursorFactory factory,
			int version) {
	...
	public void insert(int difficulty, int level, ArrayList<PlaySteps> history) {
		...
		db.execSQL("DELETE FROM " + HistoryData.HISTORY_TABLE_NAME + " WHERE "
				+ HistoryData.DIFFICULTY + " = " + difficulty + " AND "
				+ HistoryData.LEVEL + "=" + level + " ");
	...
	public ArrayList<PlaySteps> queryHistory(int difficulty, int level) {
		...
		while (cursor.moveToNext()) {
			...
			String selectionArgs2[] = new String[] { difficulty + "",
					level + "", cursor.getString(0) };
			Cursor cursor2 = db.rawQuery(query2.toString(), selectionArgs2);

			while (cursor2.moveToNext()) {

				Log.d("DaoImpl-insert", "cursor2=" + cursor2.getString(0) + ","
						+ cursor2.getString(1) + "," + cursor2.getString(2)
						+ "," + cursor2.getString(3) + ";");
				PlaySteps play = new PlaySteps();

				play.add(cursor2.getInt(0), cursor2.getInt(1), cursor2
						.getInt(2), cursor2.getInt(3));
