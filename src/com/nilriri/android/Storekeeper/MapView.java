...
public class MapView extends View {
	...
	public MapView(Context context, AttributeSet attrs) {
		...
		for (int i = 0; i < maps.length; i++) {
			...
			Log.d(" maps[i].trim()", "  maps[i].trim()= " + maps[i].trim()
					+ "=" + maps[i].trim().length());
		...
		// Log.d("RRRRRRRRRRR", " maxCount= " + maxCount + ", rowCount= " +
		// rowCount + ", mItemSize= " + mItemSize);
		// Log.d("XXXXXXXXXXXX", " mXTileCount= " + mXTileCount +
		// ", mYTileCount= " + mYTileCount + ", mItemSize= " + mItemSize);
		/*
		 * mXTileCount = (int) Math.floor(this.getWidth() / mItemSize);
		 * mYTileCount = (int) Math.floor(this.getHeight() / mItemSize); if
		 * (maxCount > mXTileCount || maxCount > mYTileCount) {
		 * 
		 * mItemSize = (int) Math.floor(this.getWidth() / maxCount);
		 * 
		 * String msg =
		 * mContext.getResources().getString(R.string.change_iconsize); msg +=
		 * " (" + mItemSize + " x " + mItemSize + ") ";
	...
	public void loadMapItem(int key, Drawable tile) {
		Bitmap bitmap = Bitmap.createBitmap(mItemSize, mItemSize,
	...
	public void onDraw(Canvas canvas) {
		...
		for (int x = 0; x < mXTileCount; x += 1) {
			for (int y = 0; y < mYTileCount; y += 1) {
				if (mMap[x][y] > 0) {
					canvas.drawBitmap(mMapItemArray[mMap[x][y]], mXOffset + x
							* mItemSize, mYOffset + y * mItemSize, mPaint);
				} else {
					canvas.drawBitmap(mMapItemArray[9], mXOffset + x
							* mItemSize, mYOffset + y * mItemSize, mPaint);
				}
			}
		}
	...
	public void resetItemSize(int w, int h, int mLevel) {
		...
		Log.d("StoreKeeperView1111", "colCount= " + colCount + ", rowCount= "
				+ rowCount + ", mItemSize= " + mItemSize);
		Log
				.d("StoreKeeperView1111", "mXTileCount= " + mXTileCount
						+ ", mYTileCount= " + mYTileCount + ", mItemSize= "
						+ mItemSize);
		...
		if (colCount >= mXTileCount || rowCount >= mYTileCount) {
			...
			resetTiles(10);
			loadMapItem(Common.LINE, mContext.getResources().getDrawable(
					R.drawable.l));
			loadMapItem(Common.HOME, mContext.getResources().getDrawable(
					R.drawable.h));
			loadMapItem(Common.YES, mContext.getResources().getDrawable(
					R.drawable.y));
			loadMapItem(Common.NO, mContext.getResources().getDrawable(
					R.drawable.n));
			loadMapItem(Common.MAN, mContext.getResources().getDrawable(
					R.drawable.m));
			loadMapItem(Common.BLANK, mContext.getResources().getDrawable(
					R.drawable.b));

		}

		Log
				.d("StoreKeeperView", "mXTileCount= " + mXTileCount
						+ ", mYTileCount= " + mYTileCount + ", mItemSize= "
						+ mItemSize);
	...
	public void setMapItem(int tileindex, int x, int y) {
		...
		if (mMap != null) {
			// Log.d("TTTTTTTTT", " mXTileCount= " + mXTileCount +
