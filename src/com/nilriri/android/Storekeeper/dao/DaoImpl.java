...
public abstract class DaoImpl extends SQLiteOpenHelper {
	...
	public DaoImpl(Context context, String name, CursorFactory factory,
			int version) {
	...
	public void onCreateBible(SQLiteDatabase db) {
		...
		try {
			...
			query.append("CREATE TABLE IF NOT EXISTS "
					+ HistoryData.HISTORY_TABLE_NAME + " (");
			query.append("    " + HistoryData._ID
	...
	public void onCreateIndex(SQLiteDatabase db) {
		try {
			db.execSQL("CREATE INDEX idx_" + HistoryData.HISTORY_TABLE_NAME
					+ "_01 ON " + HistoryData.HISTORY_TABLE_NAME
					+ "  (difficulty, level, seq ) ");
			db.execSQL("CREATE INDEX idx_" + HistoryData.HISTORY_TABLE_NAME
					+ "_02 ON " + HistoryData.HISTORY_TABLE_NAME
					+ "  (difficulty, level, seq, step ) ");
	...
	public void onInsertData(SQLiteDatabase db) {
		...
		String sqls[] = mContext.getResources().getStringArray(
				R.array.sample_play);
		db.beginTransaction();
		try {
			db.execSQL("delete from " + HistoryData.BACKUP_TABLE_NAME + "");
			db.execSQL("insert into " + HistoryData.BACKUP_TABLE_NAME
					+ " select * from " + HistoryData.HISTORY_TABLE_NAME + "");
			...
			db.execSQL("insert into " + HistoryData.HISTORY_TABLE_NAME
					+ " select * from " + HistoryData.BACKUP_TABLE_NAME + "");
